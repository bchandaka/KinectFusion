...
class KinectDataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        rgb_path = path.join(rootdir, "rgb")
        depth_path = path.join(rootdir, "depth")
        ts = [int(i.split('.')[0]) for i in sorted(listdir(path.join(rootdir, "depth")))]
        ...
        for i, stamp in enumerate(tqdm(ts)):
            ...
            intrinsics, c2w = arr2K(get_calib()['kinect']), np.eye(4, dtype=np.float32)
            c2w = torch.tensor(c2w, dtype=torch.float32)
            # read images
            rgb = np.array(imageio.imread(path.join(rgb_path, f"{stamp}.png"))).astype(np.float32)
            depth = np.load(path.join(depth_path, f"{stamp}.npy")).astype(np.float32)
